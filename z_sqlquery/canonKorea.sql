...
CREATE TABLE REPLY_TBL(
  ...
  REF_LIKE NUMBER DEFAULT 0,
  REF_UNLIKE NUMBER DEFAULT 0
);
...
ALTER TABLE REPLY_TBL ADD REF_LIKE NUMBER DEFAULT 0;
ALTER TABLE REPLY_TBL ADD REF_UNLIKE NUMBER  DEFAULT 0;
UPDATE REPLY_TBL SET REF_LIKE = REF_LIKE + 1 WHERE REPLY_NO = 1;
ALTER TABLE REPLY_TBL DROP COLUMN REF_LIKE;
ALTER TABLE REPLY_TBL DROP COLUMN  REF_UNLIKE;

UPDATE REPLY_TBL SET REF_LIKE = REF_LIKE + 1 WHERE REPLY_NO = 1;
UPDATE REPLY_TBL SET REF_UNLIKE = REF_UNLIKE + 1 WHERE REPLY_NO = 1;
...
-- 좋아요싫어요를 댓글에 달면? 댓글테이블에 컬럼을추가할건지? 세로운테이블을 만들건지?
